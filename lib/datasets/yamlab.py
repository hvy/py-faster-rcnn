...
class COCOYamlab(COCO):
    def __init__(self, ann_file):
        COCO.__init__(self, ann_file)

        # Override self.cats
        categories = {}
        categories[1] = { 'supercategory': 'yamlab', 'id': 1, 'name': 'className1'}
        categories[2] = { 'supercategory': 'yamlab', 'id': 2, 'name': 'className2'}
        categories[3] = { 'supercategory': 'yamlab', 'id': 3, 'name': 'className3'}
        categories[4] = { 'supercategory': 'yamlab', 'id': 4, 'name': 'className4'}
        self.cats = categories


    # Override
    def getCatIds(self):
        print 'Yamlab: lib/datasets/yamlab.py: getCatIds()'
        return [1, 2, 3, 4]
...
class coco(imdb):
    def __init__(self, image_set, year):
        ...
        # Yamlab
        self._data_path = osp.join(cfg.DATA_DIR, 'yamlab')

        print 'datasets/yamlab.py'
        print 'self._data_path =', self._data_path

        # load COCO API, classes, class <-> id mappings
        self._COCO = COCOYamlab(self._get_ann_file())

        # cats = self._COCO.loadCats(self._COCO.getCatIds())  # original
        cats = self._COCO.cats

        print '---------------------------------------------------------------'
        print 'lib/datasets/yamlab.py'
        print 'cats'
        print cats
        print '---------------------------------------------------------------'

        self._classes = tuple(['__background__'] + [c[1]['name'] for c in cats.iteritems()])
        ...
        self._class_to_coco_cat_id = dict(zip([c[1]['name'] for c in cats.iteritems()],
    ...
    def _get_ann_file(self):
        ...
        ann_file = osp.join(self._data_path, 'annotations',
    ...
    def _load_image_set_index(self):
        ...
        """
        # Yamlab Note
        # Get only the image Ids that we need?
